...
| 2 · Lesion-scale QC metric | **blocked** (metric now **implemented**) | The lesion-scale metric is built + fail-closed (`registration_qc_service.lesion_scale_qc`): worst-case stable-fiducial centroid residual `R_disp` (mm), mutual-best matching, a coverage guard for the rotational lever arm. It supersedes whole-brain Dice. But it stays blocked on MEASURED grounds: **only 1/40 MSLesSeg pairs is measurable** (MS morphology rarely gives ≥3 large stable well-spread fiducials) and that pair's jitter floor **1.33 mm > the 1.0 mm cut** at zero registration error. The registrar-in-the-loop test + NCC arm are FLAIR-blocked. |
