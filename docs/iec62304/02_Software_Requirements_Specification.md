...
| REQ-FUNC-LONG-001 | Longitudinal TP2→TP1 registration shall be RIGID (Euler3D) only; deformable/non-linear registration shall never be used (it can erase real lesion change), enforced by a static test. Registration shall be deterministic (single-threaded) and FAIL-CLOSED (non-finite / non-3D / degenerate brain mask / non-convergence / error → registration_ok=false → caller keeps the candidate framing). | Must | C | Test | HAZ-LONG-1, HAZ-LONG-2 |
| REQ-FUNC-LONG-002 | The registration service shall run in SHADOW: `registration_verified` shall never be set true by it, and the longitudinal endpoint + report builder shall keep candidate framing regardless. The whole-brain-overlap QC readout is advisory and shall NOT be presented as the safety/flip gate (it saturates above the lesion length-scale). | Must | C | Test + usability | HAZ-LONG-1, HAZ-LONG-3 |
| REQ-FUNC-LONG-010 | The system shall not assert VERIFIED longitudinal change (`LONG_REGISTRATION_VERIFIED_ENABLED=false`) until every gate in the V&V dossier (`docs/longitudinal-registration/reg_vv_gate_status.json`) is `pass`; a CI test shall fail the build if the flag is enabled while any gate is not green. Gates 4–5 are NOT-MEETABLE on in-hand data (no adjudicated multi-session intensity pairs). | Must | C | Test | HAZ-LONG-1, HAZ-LONG-3 |
| REQ-FUNC-LONG-011 | The registration QC gate for the flip shall be at LESION scale (worst-case matched-stable-fiducial centroid residual in mm), with mutual-best matching (no greedy neighbour false-match) and a coverage guard for the rotational lever arm, and shall FAIL CLOSED on <3 fiducials / no coverage / any undefined case. Whole-brain-overlap Dice shall NOT be the flip QC (it saturates above the lesion scale). | Must | C | Test | HAZ-LONG-3 |
