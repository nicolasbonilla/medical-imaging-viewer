...
| RC-017 | HAZ-010 | JWT authentication on ALL API endpoints (33 Class C + 52 other = 85 total) | All route files import get_current_active_user; every async def endpoint has auth dependency | Code inspection: grep -c get_current_active_user across all route files | VERIFIED |
