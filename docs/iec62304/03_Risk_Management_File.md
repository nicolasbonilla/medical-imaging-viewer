...
| HAZ-LONG-1 | A misregistered timepoint pair mints a phantom "new" lesion → false dissemination-in-time → wrong DMT escalation | Serious | RC-LONG-1: registration runs in SHADOW (`registration_verified` never true, REQ-FUNC-LONG-002), the candidate firewall + report `verified=False` guard stay intact, and the VERIFIED-change flip is CI-gated dark until a lesion-scale QC gate + validation exist (NOT-MEETABLE on in-hand data). Registration reduces (does not certify) misregistration in the candidate counts | Lesion-scale QC metric + false-accept measurement on adjudicated multi-session intensity pairs (dossier gates 2/4) |
| HAZ-LONG-2 | Deformable/non-linear registration warps TP2 to match TP1, shrinking/erasing a real new lesion (the classic longitudinal-MS trap) | Serious | RC-LONG-2: RIGID-only (REQ-FUNC-LONG-001); a static CI test asserts no deformable-transform symbol appears in registration_service | test_registration_failclosed.py::test_no_deformable_registration_symbol_present |
| HAZ-LONG-3 | Whole-brain-overlap Dice QC PASSES on a lesion-scale (3–5 mm) misregistration (it saturates above the lesion length-scale) → over-trust of alignment | Serious | RC-LONG-3: the whole-brain QC is ADVISORY only and is explicitly NOT the flip gate. A lesion-scale fail-closed metric (`registration_qc_service.lesion_scale_qc`, REQ-FUNC-LONG-011) now supersedes it — worst-case stable-fiducial centroid residual (mm), mutual-best matching, coverage guard. The flip stays blocked (dossier gate 2): the measured jitter floor (1.33 mm on the only measurable MSLesSeg pair) exceeds the 1.0 mm cut and only 1/40 pairs is certifiable, so the metric is non-certifying on MS morphology | Lesion-scale metric implemented + fail-closed-tested; registrar-in-the-loop (Exp B) validation FLAIR-blocked; keep flip dark (adversarial refutes w3p81u3kz + wf_bf51d924-57d) |
