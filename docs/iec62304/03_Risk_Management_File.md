...
| HAZ-LONG-3 | Whole-brain-overlap Dice QC PASSES on a lesion-scale (3–5 mm) misregistration (it saturates above the lesion length-scale) → over-trust of alignment | Serious | RC-LONG-3: the whole-brain QC is ADVISORY only and is explicitly NOT the flip gate; the flip stays blocked (dossier gate 2) until a lesion-scale QC metric is designed + validated | Design + validate a lesion-neighbourhood QC metric before any flip (adversarial refute, workflow w3p81u3kz) |
