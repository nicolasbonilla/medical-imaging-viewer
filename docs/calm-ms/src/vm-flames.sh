...
FILTER=$(meta attributes/case-filter); FILTER=${FILTER:-}    # comma-sep caseids to process ONLY (backfill)
...
for split in root:
    for pat in sorted(glob.glob(f"{split}/P*")):
        tps=[d for d in sorted(glob.glob(f"{pat}/T*")) if os.path.isdir(d)]   # train: per-timepoint dirs
        if tps:
            for tp in tps:
        ...
        else:                                                                  # test: flat P{n}/P{n}_FLAIR
            fl=glob.glob(f"{pat}/*FLAIR*.nii.gz"); gt=glob.glob(f"{pat}/*MASK*.nii.gz")
            if fl and gt: add(f"mslesseg_{os.path.basename(pat)}", fl[0], gt[0])

cases=sorted(gt_map)
if FILT: cases=[c for c in cases if c in FILT]              # backfill: only these caseids
