    ...
    return (
      <div className="h-screen w-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-gray-400 text-lg">{t('auth.verifyingAuth', 'Verifying authentication...')}</p>
        </div>
      </div>
    );
  }
      ...
      return (
        <div className="h-screen w-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
          <div className="max-w-md p-8 bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 text-center">
            <h2 className="text-2xl font-bold text-white mb-4">{t('auth.accessDenied', 'Access Denied')}</h2>
            <p className="text-gray-300 mb-6">
              {t('auth.insufficientPermissions', "You don't have sufficient permissions to access this page.")}
            </p>
            <p className="text-sm text-gray-400">
              {t('auth.requiredRole', 'Required role:')} <span className="text-blue-400 font-semibold">{requiredRole}</span>
              <br />
              {t('auth.yourRole', 'Your role:')} <span className="text-blue-400 font-semibold">{user.role}</span>
            </p>
            <button
